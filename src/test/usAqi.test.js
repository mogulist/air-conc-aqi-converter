...
describe(`${aqiName}: PM2.5 농도 -> US AQI 테스트`, () => {
    ...
    it('농도 35.4 는 AQI 100이다.', () => {
        assert.equal(getAqiFromConc(aqiName, 'pm25', 35.4), 100)
    })
...
describe(`${aqiName}: PM10 농도 -> US AQI 테스트`, () => {
    it('농도 54 는 AQI 50이다.', () => {
        assert.equal(getAqiFromConc(aqiName, 'pm10', 54), 50)
    })
    it('농도 154 는 AQI 100이다.', () => {
        assert.equal(getAqiFromConc(aqiName, 'pm10', 154), 100)
    })
    it('농도 254 는 AQI 150이다.', () => {
        assert.equal(getAqiFromConc(aqiName, 'pm10', 254), 150)
    })
...
describe(`${aqiName}: NO2 농도 -> US AQI 테스트`, () => {
    ...
    it(`농도 0.085 는 AQI 84 이다.`, () => {
        assert.equal(getAqiFromConc(aqiName, pollutant, 0.085), 84)
    })
    it(`농도 0.234 는 AQI 126 이다.`, () => {
        assert.equal(getAqiFromConc(aqiName, pollutant, 0.234), 126)
    })
    it(`농도 1.739 는 AQI 423 이다.`, () => {
        assert.equal(getAqiFromConc(aqiName, pollutant, 1.739), 423)
    })
...
describe(`${aqiName}: O3 농도 -> US AQI 테스트`, () => {
    ...
    it(`농도 0.083 는 AQI 78 이다.`, () => {
        assert.equal(getAqiFromConc(aqiName, pollutant, 0.083), 78)
    })
    it(`농도 0.193 는 AQI 186 이다.`, () => {
        assert.equal(getAqiFromConc(aqiName, pollutant, 0.193), 186)
    })
