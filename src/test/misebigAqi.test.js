...
describe('PM2.5 농도 -> 미세빅AQI 테스트', () => {
    ...
    it('농도 25.4 는 AQI 100이다.', () => {
        assert.equal(getConcFromAqi(aqiName, 'pm25', 25.4), 100)
    })
...
describe('PM10 농도 -> 미세빅AQI 테스트', () => {
    it('농도 30 는 AQI 50이다.', () => {
        assert.equal(getConcFromAqi(aqiName, 'pm10', 30), 50)
    })
    it('농도 50 는 AQI 100이다.', () => {
        assert.equal(getConcFromAqi(aqiName, 'pm10', 50), 100)
    })
    it('농도 150 는 AQI 150이다.', () => {
        assert.equal(getConcFromAqi(aqiName, 'pm10', 150), 150)
    })
